...
function App() {
  ...
  var assignment = _useState1[0];
  var setAssignment = _useState1[1];

  var _useState2 = useState("strict");
  var teacherType = _useState2[0];
  var setTeacherType = _useState2[1];

  var _useState3 = useState(null);
  var result = _useState3[0];
  var setResult = _useState3[1];
  ...
  var charCount = _useState6[0];
  var setCharCount = _useState6[1];

  var _useState7 = useState(false);
  var showResult = _useState7[0];
  var setShowResult = _useState7[1];

  var _useState8 = useState("input");
  var activeTab = _useState8[0];
  var setActiveTab = _useState8[1];

  useEffect(function() {
    setCharCount(assignment.length);
  }, [assignment]);

  useEffect(function() {
    if (result) {
      setTimeout(function() { setShowResult(true); }, 100);
    } else {
      setShowResult(false);
    }
  }, [result]);
  ...
  var handleSubmit = async function() {
    ...
    try {
      ...
      setError("Something went wrong. Make sure your backend server is running on port 5000.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="pg-app">
        ...
        <nav className="pg-nav">
          <div className="pg-nav-inner">
            ...
            <div className="pg-nav-links">
              ...
              <span className="pg-nav-version">v1.0</span>
            </div>
          </div>
        </nav>
        ...
        <header className="pg-hero">
          ...
          <p className="pg-hero-sub">
            Paste your assignment, pick a teacher personality, and get instant AI feedback with a detailed grade breakdown.
          </p>
        </header>
...
.pg-orb-1 {
  width: 600px; height: 600px; background: #7c3aed; top: -200px; left: -100px;
  animation: pg-float 20s ease-in-out infinite;
}
.pg-orb-2 {
  width: 500px; height: 500px; background: #2563eb; bottom: -150px; right: -100px;
  animation: pg-float 25s ease-in-out infinite reverse;
}
.pg-orb-3 {
  width: 300px; height: 300px; background: #06b6d4; top: 40%; left: 60%;
  animation: pg-float 18s ease-in-out infinite 3s;
}
@keyframes pg-float {
...
.pg-app {
  position: relative; z-index: 1; max-width: 820px; margin: 0 auto; padding: 0 24px;
}
...
.pg-logo-mark {
  ...
  font-family: 'Instrument Serif', serif; font-size: 22px; color: white; font-weight: 400;
}
.pg-logo-text {
...
.pg-fade-in {
  animation: pg-fadeUp 0.5s ease forwards;
}
@keyframes pg-fadeUp {
...
.pg-char-count {
  position: absolute; bottom: 12px; right: 16px; font-size: 12px;
...
.pg-strength-item + .pg-strength-item {
...
.pg-deduction-item + .pg-deduction-item {
  border-top: 1px solid rgba(255,255,255,0.04);
}
.pg-deduction-top {
...
.pg-suggestion-item + .pg-suggestion-item {
  border-top: 1px solid rgba(255,255,255,0.04);
}
.pg-suggestion-num {
